...
///===================================================
/// 1. 网络可达性是否有必要先检测？
/// 2. get和post分开处理，对于POST的结果，默认不进行保存!除非用户自己保存
/// 3. 缓存的时候是否还有必要调用什么其他第三方解析框架，根据自己的业务，可以直接强制默认返回的就是Dictionary.
/// 4. 缓存方案的考虑，Archive好还是SQL，又或者plist文件
/// 5. 增加一个文件上传的接口，也就是至少get/post, post文件两个
/// 6. 用通知方便，还是block回调方便?
/// 7. 对于错误怎么处理，是放在这个中间层直接处理了，还是到vc自己处理?  -- 通用错误，比如没网络直接Tool处理，但是照样传递error给上一层，至于上一层处不处理另外说？（是上一层！！！不是vc）
///===================================================
